...
const SalesTable = ({ route, sales, setSale, setEdit }) => {
    ...
    const handleDelete = e => {
        ...
        //Fetch to delete sale details
        fetch(`${route}/api/sale/details/${e.target.id}?accesstoken=${cookies.get('token')}`, requestInit)
        .then(res => res.json())
        .then(() => {})
        .catch(error => {
            console.log('Error eliminando la venta: ' + error)
    ...
    const handleUpdate = e => {
        ...
        .then(data => {
            ...
            cookies.set('idSale', e.target.id, {path: "/"})
        })
        .catch(error => {
